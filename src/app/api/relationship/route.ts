...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ({ usedFree } = await consumeQuestionCredit(user.id, "relationship"));
    } catch (err) {
    ...
    try {
      ...
      await refundQuestionCredit(user.id, usedFree, "relationship");
      console.error("[relationship] AI generation failed, credit refunded", err);
